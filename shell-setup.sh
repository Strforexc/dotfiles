...
if [[ ! -f $HOME/bin/diff-so-fancy ]]; then
    curl -o $HOME/bin/diff-so-fancy https://github.com/so-fancy/diff-so-fancy/master/third_party/build_fatpack/diff-so-fancy
...
if command -v nvim > /dev/null; then
    ...
    mkdir -p $NVIM/bin
    cd $NVIM/bin
    curl -LO https://github.com/neovim/neovim/releases/download/nightly/nvim-linux-x86_64.appimage -o nvim.appimage
...
if [[ ! -d $NVIM/node ]]; then
    ...
    curl -sL https://raw.githubusercontent.com/vercel/install-node/refs/heads/master/install.sh -o $NODE_SCRIPT
    chmod +x $NODE_SCRIPT
